#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: SpawnUI

#include "Basic.hpp"

#include "SpawnUI_classes.hpp"
#include "SpawnUI_parameters.hpp"


SDK_NAMESPACE_START

// Function SpawnUI.SpawnUI_C.AddedToViewport
// (Event, Public, BlueprintEvent)

void USpawnUI_C::AddedToViewport()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "AddedToViewport");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.AssignBedsButtonSelect
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class UDataListButton_ASA_C*            Data                                                   (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::AssignBedsButtonSelect(class UDataListButton_ASA_C* Data)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "AssignBedsButtonSelect");

	Params::SpawnUI_C_AssignBedsButtonSelect Parms{};

	Parms.Data = Data;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.AssignBedSpawnSelect
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class UBedSpawnWidget_ASA_C*            SpawnRegion                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::AssignBedSpawnSelect(class UBedSpawnWidget_ASA_C* SpawnRegion)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "AssignBedSpawnSelect");

	Params::SpawnUI_C_AssignBedSpawnSelect Parms{};

	Parms.SpawnRegion = SpawnRegion;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.AssignSpawnRegionSelect
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class USpawnRegionWidget_ASA_C*         SpawnRegion                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::AssignSpawnRegionSelect(class USpawnRegionWidget_ASA_C* SpawnRegion)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "AssignSpawnRegionSelect");

	Params::SpawnUI_C_AssignSpawnRegionSelect Parms{};

	Parms.SpawnRegion = SpawnRegion;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_BodyTemplate_Slider_K2Node_ComponentBoundEvent_19_ReturnValue__DelegateSignature
// (BlueprintEvent)
// Parameters:
// const class FText&                      ReturnValue                                            (BlueprintVisible, BlueprintReadOnly, Parm)
// int32                                   Index_0                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_BodyTemplate_Slider_K2Node_ComponentBoundEvent_19_ReturnValue__DelegateSignature(const class FText& ReturnValue, int32 Index_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_BodyTemplate_Slider_K2Node_ComponentBoundEvent_19_ReturnValue__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_BodyTemplate_Slider_K2Node_ComponentBoundEvent_19_ReturnValue__DelegateSignature Parms{};

	Parms.ReturnValue = std::move(ReturnValue);
	Parms.Index_0 = Index_0;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_CloseButton_2_K2Node_ComponentBoundEvent_0_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_CloseButton_2_K2Node_ComponentBoundEvent_0_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_CloseButton_2_K2Node_ComponentBoundEvent_0_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_CreateNewPlayerButton_K2Node_ComponentBoundEvent_5_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_CreateNewPlayerButton_K2Node_ComponentBoundEvent_5_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_CreateNewPlayerButton_K2Node_ComponentBoundEvent_5_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_DataListButtonVariable_HairSylePicker_Widget_ASA_K2Node_ComponentBoundEvent_13_ReturnValue__DelegateSignature
// (BlueprintEvent)
// Parameters:
// const class FText&                      ReturnValue                                            (BlueprintVisible, BlueprintReadOnly, Parm)
// int32                                   Index_0                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_DataListButtonVariable_HairSylePicker_Widget_ASA_K2Node_ComponentBoundEvent_13_ReturnValue__DelegateSignature(const class FText& ReturnValue, int32 Index_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_DataListButtonVariable_HairSylePicker_Widget_ASA_K2Node_ComponentBoundEvent_13_ReturnValue__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_DataListButtonVariable_HairSylePicker_Widget_ASA_K2Node_ComponentBoundEvent_13_ReturnValue__DelegateSignature Parms{};

	Parms.ReturnValue = std::move(ReturnValue);
	Parms.Index_0 = Index_0;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_debuglightsbutton_K2Node_ComponentBoundEvent_12_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_debuglightsbutton_K2Node_ComponentBoundEvent_12_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_debuglightsbutton_K2Node_ComponentBoundEvent_12_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_EyeColor_Slider_K2Node_ComponentBoundEvent_15_ChangedColor__DelegateSignature
// (BlueprintEvent)
// Parameters:
// const struct FLinearColor&              Color                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_EyeColor_Slider_K2Node_ComponentBoundEvent_15_ChangedColor__DelegateSignature(const struct FLinearColor& Color)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_EyeColor_Slider_K2Node_ComponentBoundEvent_15_ChangedColor__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_EyeColor_Slider_K2Node_ComponentBoundEvent_15_ChangedColor__DelegateSignature Parms{};

	Parms.Color = std::move(Color);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_FacialHairColor_K2Node_ComponentBoundEvent_8_ChangedColor__DelegateSignature
// (BlueprintEvent)
// Parameters:
// const struct FLinearColor&              Color                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_FacialHairColor_K2Node_ComponentBoundEvent_8_ChangedColor__DelegateSignature(const struct FLinearColor& Color)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_FacialHairColor_K2Node_ComponentBoundEvent_8_ChangedColor__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_FacialHairColor_K2Node_ComponentBoundEvent_8_ChangedColor__DelegateSignature Parms{};

	Parms.Color = std::move(Color);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_FacialHairStyle_Slider_K2Node_ComponentBoundEvent_6_ReturnValue__DelegateSignature
// (BlueprintEvent)
// Parameters:
// const class FText&                      ReturnValue                                            (BlueprintVisible, BlueprintReadOnly, Parm)
// int32                                   Index_0                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_FacialHairStyle_Slider_K2Node_ComponentBoundEvent_6_ReturnValue__DelegateSignature(const class FText& ReturnValue, int32 Index_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_FacialHairStyle_Slider_K2Node_ComponentBoundEvent_6_ReturnValue__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_FacialHairStyle_Slider_K2Node_ComponentBoundEvent_6_ReturnValue__DelegateSignature Parms{};

	Parms.ReturnValue = std::move(ReturnValue);
	Parms.Index_0 = Index_0;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_FemaleButton_K2Node_ComponentBoundEvent_3_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_FemaleButton_K2Node_ComponentBoundEvent_3_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_FemaleButton_K2Node_ComponentBoundEvent_3_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_HairColor_Slider_K2Node_ComponentBoundEvent_16_ChangedColor__DelegateSignature
// (BlueprintEvent)
// Parameters:
// const struct FLinearColor&              Color                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_HairColor_Slider_K2Node_ComponentBoundEvent_16_ChangedColor__DelegateSignature(const struct FLinearColor& Color)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_HairColor_Slider_K2Node_ComponentBoundEvent_16_ChangedColor__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_HairColor_Slider_K2Node_ComponentBoundEvent_16_ChangedColor__DelegateSignature Parms{};

	Parms.Color = std::move(Color);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_Header_GridPanel_K2Node_ComponentBoundEvent_10_OnSelectionChangedIndex__DelegateSignature
// (BlueprintEvent)
// Parameters:
// int32                                   Index_0                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_Header_GridPanel_K2Node_ComponentBoundEvent_10_OnSelectionChangedIndex__DelegateSignature(int32 Index_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_Header_GridPanel_K2Node_ComponentBoundEvent_10_OnSelectionChangedIndex__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_Header_GridPanel_K2Node_ComponentBoundEvent_10_OnSelectionChangedIndex__DelegateSignature Parms{};

	Parms.Index_0 = Index_0;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_intensitySlider_K2Node_ComponentBoundEvent_11_OnFloatValueChangedEvent__DelegateSignature
// (BlueprintEvent)
// Parameters:
// float                                   Value                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_intensitySlider_K2Node_ComponentBoundEvent_11_OnFloatValueChangedEvent__DelegateSignature(float Value)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_intensitySlider_K2Node_ComponentBoundEvent_11_OnFloatValueChangedEvent__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_intensitySlider_K2Node_ComponentBoundEvent_11_OnFloatValueChangedEvent__DelegateSignature Parms{};

	Parms.Value = Value;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_MainMinimapUI_ASA_K2Node_ComponentBoundEvent_2_Zoom__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_MainMinimapUI_ASA_K2Node_ComponentBoundEvent_2_Zoom__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_MainMinimapUI_ASA_K2Node_ComponentBoundEvent_2_Zoom__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_MaleButton_K2Node_ComponentBoundEvent_4_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_MaleButton_K2Node_ComponentBoundEvent_4_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_MaleButton_K2Node_ComponentBoundEvent_4_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_OverallHeightSlider_K2Node_ComponentBoundEvent_17_OnFloatValueGenericChangedEvent__DelegateSignature
// (BlueprintEvent)
// Parameters:
// float                                   Value                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_OverallHeightSlider_K2Node_ComponentBoundEvent_17_OnFloatValueGenericChangedEvent__DelegateSignature(float Value)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_OverallHeightSlider_K2Node_ComponentBoundEvent_17_OnFloatValueGenericChangedEvent__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_OverallHeightSlider_K2Node_ComponentBoundEvent_17_OnFloatValueGenericChangedEvent__DelegateSignature Parms{};

	Parms.Value = Value;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_0_OnSelectionChangedEvent__DelegateSignature
// (BlueprintEvent)
// Parameters:
// const class FString&                    SelectedItem                                           (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, HasGetValueTypeHash)
// ESelectInfo                             SelectionType                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_0_OnSelectionChangedEvent__DelegateSignature(const class FString& SelectedItem, ESelectInfo SelectionType)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_0_OnSelectionChangedEvent__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_0_OnSelectionChangedEvent__DelegateSignature Parms{};

	Parms.SelectedItem = std::move(SelectedItem);
	Parms.SelectionType = SelectionType;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_2_OnOpeningEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_2_OnOpeningEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_2_OnOpeningEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_4_OnClosingEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_4_OnClosingEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_PlayerVoiceSelectorCombobox_K2Node_ComponentBoundEvent_4_OnClosingEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_PlayVoiceDemoButton_K2Node_ComponentBoundEvent_1_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_PlayVoiceDemoButton_K2Node_ComponentBoundEvent_1_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_PlayVoiceDemoButton_K2Node_ComponentBoundEvent_1_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_printcharacter_K2Node_ComponentBoundEvent_18_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_printcharacter_K2Node_ComponentBoundEvent_18_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_printcharacter_K2Node_ComponentBoundEvent_18_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_RandomizeButton_K2Node_ComponentBoundEvent_3_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_RandomizeButton_K2Node_ComponentBoundEvent_3_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_RandomizeButton_K2Node_ComponentBoundEvent_3_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_ShowBedsButton_K2Node_ComponentBoundEvent_1_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_ShowBedsButton_K2Node_ComponentBoundEvent_1_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_ShowBedsButton_K2Node_ComponentBoundEvent_1_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_SkinColor_Slider_K2Node_ComponentBoundEvent_14_ChangedColor__DelegateSignature
// (BlueprintEvent)
// Parameters:
// const struct FLinearColor&              Color                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_SkinColor_Slider_K2Node_ComponentBoundEvent_14_ChangedColor__DelegateSignature(const struct FLinearColor& Color)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_SkinColor_Slider_K2Node_ComponentBoundEvent_14_ChangedColor__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_SkinColor_Slider_K2Node_ComponentBoundEvent_14_ChangedColor__DelegateSignature Parms{};

	Parms.Color = std::move(Color);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_Slider_53_K2Node_ComponentBoundEvent_3_OnFloatValueChangedEvent__DelegateSignature
// (BlueprintEvent)
// Parameters:
// float                                   Value                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BndEvt__SpawnUI_Slider_53_K2Node_ComponentBoundEvent_3_OnFloatValueChangedEvent__DelegateSignature(float Value)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_Slider_53_K2Node_ComponentBoundEvent_3_OnFloatValueChangedEvent__DelegateSignature");

	Params::SpawnUI_C_BndEvt__SpawnUI_Slider_53_K2Node_ComponentBoundEvent_3_OnFloatValueChangedEvent__DelegateSignature Parms{};

	Parms.Value = Value;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BndEvt__SpawnUI_TogglePhotoMode_K2Node_ComponentBoundEvent_7_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void USpawnUI_C::BndEvt__SpawnUI_TogglePhotoMode_K2Node_ComponentBoundEvent_7_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BndEvt__SpawnUI_TogglePhotoMode_K2Node_ComponentBoundEvent_7_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BP_FinalAdjustPlayerCharacterConfigBeforeSpawn
// (Event, Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// const struct FPrimalPlayerCharacterConfigStruct&InPlayerCharacterConfig                                (BlueprintVisible, BlueprintReadOnly, Parm)
// struct FPrimalPlayerCharacterConfigStructReturnValue                                            (Parm, OutParm, ReturnParm)

struct FPrimalPlayerCharacterConfigStruct USpawnUI_C::BP_FinalAdjustPlayerCharacterConfigBeforeSpawn(const struct FPrimalPlayerCharacterConfigStruct& InPlayerCharacterConfig)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BP_FinalAdjustPlayerCharacterConfigBeforeSpawn");

	Params::SpawnUI_C_BP_FinalAdjustPlayerCharacterConfigBeforeSpawn Parms{};

	Parms.InPlayerCharacterConfig = std::move(InPlayerCharacterConfig);

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.BP_OnUpdateCharacterPreviewImage_Pre
// (Event, Public, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::BP_OnUpdateCharacterPreviewImage_Pre()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BP_OnUpdateCharacterPreviewImage_Pre");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.BPCreatedSpawnRegionWidget
// (Event, Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   SpawnRegionIndex                                       (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const struct FVector&                   Location                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   Difficult                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BPCreatedSpawnRegionWidget(int32 SpawnRegionIndex, const struct FVector& Location, int32 Difficult)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BPCreatedSpawnRegionWidget");

	Params::SpawnUI_C_BPCreatedSpawnRegionWidget Parms{};

	Parms.SpawnRegionIndex = SpawnRegionIndex;
	Parms.Location = std::move(Location);
	Parms.Difficult = Difficult;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BPExecutedCommandForPrimalUI
// (Event, Public, HasOutParams, BlueprintEvent)
// Parameters:
// class FName                             CommandName                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const struct FBPNetExecParams&          ExecParams                                             (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm)

void USpawnUI_C::BPExecutedCommandForPrimalUI(class FName CommandName, const struct FBPNetExecParams& ExecParams)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BPExecutedCommandForPrimalUI");

	Params::SpawnUI_C_BPExecutedCommandForPrimalUI Parms{};

	Parms.CommandName = CommandName;
	Parms.ExecParams = std::move(ExecParams);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BPRemoveBed
// (Event, Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   SpawnPointID                                           (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BPRemoveBed(int32 SpawnPointID)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BPRemoveBed");

	Params::SpawnUI_C_BPRemoveBed Parms{};

	Parms.SpawnPointID = SpawnPointID;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BPSetSelectedSpawnRegion
// (Event, Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   SpawnRegionIndex                                       (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const struct FVector&                   Location                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BPSetSelectedSpawnRegion(int32 SpawnRegionIndex, const struct FVector& Location)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BPSetSelectedSpawnRegion");

	Params::SpawnUI_C_BPSetSelectedSpawnRegion Parms{};

	Parms.SpawnRegionIndex = SpawnRegionIndex;
	Parms.Location = std::move(Location);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BPSetupBed
// (Event, Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   BedIndex                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   SpawnPointID                                           (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const class FString&                    BedName                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, HasGetValueTypeHash)
// const struct FVector&                   AtLocation                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// double                                  NextAllowedUseTime                                     (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// bool                                    bAllowedUse                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   NumBeds                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::BPSetupBed(int32 BedIndex, int32 SpawnPointID, const class FString& BedName, const struct FVector& AtLocation, double NextAllowedUseTime, bool bAllowedUse, int32 NumBeds)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BPSetupBed");

	Params::SpawnUI_C_BPSetupBed Parms{};

	Parms.BedIndex = BedIndex;
	Parms.SpawnPointID = SpawnPointID;
	Parms.BedName = std::move(BedName);
	Parms.AtLocation = std::move(AtLocation);
	Parms.NextAllowedUseTime = NextAllowedUseTime;
	Parms.bAllowedUse = bAllowedUse;
	Parms.NumBeds = NumBeds;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.BPSetupButtonDataForSpawnRegion
// (Event, Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   SpawnRegionIndex                                       (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const class FString&                    Text                                                   (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, HasGetValueTypeHash)
// const class FString&                    SecondText                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, HasGetValueTypeHash)

void USpawnUI_C::BPSetupButtonDataForSpawnRegion(int32 SpawnRegionIndex, const class FString& Text, const class FString& SecondText)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "BPSetupButtonDataForSpawnRegion");

	Params::SpawnUI_C_BPSetupButtonDataForSpawnRegion Parms{};

	Parms.SpawnRegionIndex = SpawnRegionIndex;
	Parms.Text = std::move(Text);
	Parms.SecondText = std::move(SecondText);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.CalculateAverageBeds
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::CalculateAverageBeds()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "CalculateAverageBeds");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.Clicked_Event
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class USpawnRegionWidget_ASA_C*         SpawnRegion                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::Clicked_Event(class USpawnRegionWidget_ASA_C* SpawnRegion)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Clicked_Event");

	Params::SpawnUI_C_Clicked_Event Parms{};

	Parms.SpawnRegion = SpawnRegion;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.Clicked_Event_0
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class UDinoListButtonWidget*            DataList                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::Clicked_Event_0(class UDinoListButtonWidget* DataList)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Clicked_Event_0");

	Params::SpawnUI_C_Clicked_Event_0 Parms{};

	Parms.DataList = DataList;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.Clicked_Event_1
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class UBedSpawnWidget_ASA_C*            BedSpawn                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::Clicked_Event_1(class UBedSpawnWidget_ASA_C* BedSpawn)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Clicked_Event_1");

	Params::SpawnUI_C_Clicked_Event_1 Parms{};

	Parms.BedSpawn = BedSpawn;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.Clicked_Event_2
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class UDinoListButtonWidget*            DataList                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::Clicked_Event_2(class UDinoListButtonWidget* DataList)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Clicked_Event_2");

	Params::SpawnUI_C_Clicked_Event_2 Parms{};

	Parms.DataList = DataList;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.CombineDynamicMaterialSlidersToFVectors
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// TArray<struct FVector>                  ReturnValue                                            (Parm, OutParm, ReturnParm)

TArray<struct FVector> USpawnUI_C::CombineDynamicMaterialSlidersToFVectors()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "CombineDynamicMaterialSlidersToFVectors");

	Params::SpawnUI_C_CombineDynamicMaterialSlidersToFVectors Parms{};

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.Construct
// (BlueprintCosmetic, Event, Public, BlueprintEvent)

void USpawnUI_C::Construct()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Construct");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.ExecuteUbergraph_SpawnUI
// (Final, UbergraphFunction, HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::ExecuteUbergraph_SpawnUI(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "ExecuteUbergraph_SpawnUI");

	Params::SpawnUI_C_ExecuteUbergraph_SpawnUI Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.FixPS5Icons
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::FixPS5Icons()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "FixPS5Icons");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.Get or Create Dynamic Material
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// class UMaterialInstanceDynamic**        Output_Get                                             (Parm, OutParm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::Get_or_Create_Dynamic_Material(class UMaterialInstanceDynamic** Output_Get)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Get or Create Dynamic Material");

	Params::SpawnUI_C_Get_or_Create_Dynamic_Material Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Output_Get != nullptr)
		*Output_Get = Parms.Output_Get;
}


// Function SpawnUI.SpawnUI_C.Get Preview Mesh Normalized Height
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// double*                                 Height                                                 (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::Get_Preview_Mesh_Normalized_Height(double* Height)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Get Preview Mesh Normalized Height");

	Params::SpawnUI_C_Get_Preview_Mesh_Normalized_Height Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Height != nullptr)
		*Height = Parms.Height;
}


// Function SpawnUI.SpawnUI_C.Get_BorderCharacterCreation_Visibility
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// ESlateVisibility                        ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

ESlateVisibility USpawnUI_C::Get_BorderCharacterCreation_Visibility()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Get_BorderCharacterCreation_Visibility");

	Params::SpawnUI_C_Get_BorderCharacterCreation_Visibility Parms{};

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.Get_OverallHeightSlider_bIsEnabled
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// bool                                    ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

bool USpawnUI_C::Get_OverallHeightSlider_bIsEnabled()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Get_OverallHeightSlider_bIsEnabled");

	Params::SpawnUI_C_Get_OverallHeightSlider_bIsEnabled Parms{};

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.GetAllColorSliders
// (Event, Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// TArray<class UDataListValueGeneric*>    ReturnValue                                            (Parm, OutParm, ReturnParm, ContainsInstancedReference)

TArray<class UDataListValueGeneric*> USpawnUI_C::GetAllColorSliders()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "GetAllColorSliders");

	Params::SpawnUI_C_GetAllColorSliders Parms{};

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.GetAllFloatSliders
// (Event, Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// TArray<class UDataListValueGeneric*>    ReturnValue                                            (Parm, OutParm, ReturnParm, ContainsInstancedReference)

TArray<class UDataListValueGeneric*> USpawnUI_C::GetAllFloatSliders()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "GetAllFloatSliders");

	Params::SpawnUI_C_GetAllFloatSliders Parms{};

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.GetCameraSettings
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   TabIndex                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// struct FCameraLocationSetting*          Return                                                 (Parm, OutParm, NoDestructor)

void USpawnUI_C::GetCameraSettings(int32 TabIndex, struct FCameraLocationSetting* Return)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "GetCameraSettings");

	Params::SpawnUI_C_GetCameraSettings Parms{};

	Parms.TabIndex = TabIndex;

	UObject::ProcessEvent(Func, &Parms);

	if (Return != nullptr)
		*Return = std::move(Parms.Return);
}


// Function SpawnUI.SpawnUI_C.GetCanvasLocationForMinimapPoint
// (Event, Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// const struct FVector&                   Location                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// struct FVector2D                        ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

struct FVector2D USpawnUI_C::GetCanvasLocationForMinimapPoint(const struct FVector& Location)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "GetCanvasLocationForMinimapPoint");

	Params::SpawnUI_C_GetCanvasLocationForMinimapPoint Parms{};

	Parms.Location = std::move(Location);

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.GetDifficultRegionString
// (Event, Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   Index_0                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class FString                           ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, HasGetValueTypeHash)

class FString USpawnUI_C::GetDifficultRegionString(int32 Index_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "GetDifficultRegionString");

	Params::SpawnUI_C_GetDifficultRegionString Parms{};

	Parms.Index_0 = Index_0;

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.GetGenericCharacterPreset
// (Event, Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   Index_0                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// struct FCharacterPreset                 ReturnValue                                            (Parm, OutParm, ReturnParm)

struct FCharacterPreset USpawnUI_C::GetGenericCharacterPreset(int32 Index_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "GetGenericCharacterPreset");

	Params::SpawnUI_C_GetGenericCharacterPreset Parms{};

	Parms.Index_0 = Index_0;

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.GetIndexSelectedBodyPart
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32*                                  Array_Index                                            (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::GetIndexSelectedBodyPart(int32* Array_Index)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "GetIndexSelectedBodyPart");

	Params::SpawnUI_C_GetIndexSelectedBodyPart Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Array_Index != nullptr)
		*Array_Index = Parms.Array_Index;
}


// Function SpawnUI.SpawnUI_C.GetRespawnPanelMap
// (Event, Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// class USpawnMapWidget*                  ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, InstancedReference, NoDestructor, HasGetValueTypeHash)

class USpawnMapWidget* USpawnUI_C::GetRespawnPanelMap()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "GetRespawnPanelMap");

	Params::SpawnUI_C_GetRespawnPanelMap Parms{};

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.HandleConsoleIcons
// (Public, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::HandleConsoleIcons()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "HandleConsoleIcons");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.HeadZoomUpdate
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::HeadZoomUpdate()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "HeadZoomUpdate");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.IsFastTravelTeleport
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// bool                                    ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

bool USpawnUI_C::IsFastTravelTeleport()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "IsFastTravelTeleport");

	Params::SpawnUI_C_IsFastTravelTeleport Parms{};

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.OnGamepadActiveChangedBP
// (Event, Public, BlueprintEvent)
// Parameters:
// bool                                    bIsGamepadActive                                       (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::OnGamepadActiveChangedBP(bool bIsGamepadActive)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "OnGamepadActiveChangedBP");

	Params::SpawnUI_C_OnGamepadActiveChangedBP Parms{};

	Parms.bIsGamepadActive = bIsGamepadActive;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.OnGamepadButtonDownEvent
// (Event, Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// const struct FGeometry&                 MyGeometry                                             (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm, IsPlainOldData, NoDestructor)
// const struct FControllerEvent&          ControllerEvent                                        (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm)
// struct FEventReply                      ReturnValue                                            (Parm, OutParm, ReturnParm)

struct FEventReply USpawnUI_C::OnGamepadButtonDownEvent(const struct FGeometry& MyGeometry, const struct FControllerEvent& ControllerEvent)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "OnGamepadButtonDownEvent");

	Params::SpawnUI_C_OnGamepadButtonDownEvent Parms{};

	Parms.MyGeometry = std::move(MyGeometry);
	Parms.ControllerEvent = std::move(ControllerEvent);

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.OnGenderChange
// (Event, Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    isFemale                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::OnGenderChange(bool isFemale)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "OnGenderChange");

	Params::SpawnUI_C_OnGenderChange Parms{};

	Parms.isFemale = isFemale;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.OnLoaded_3E84B3BE41EA3801C29ED5ABF7BB70A9
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// TSubclassOf<class UObject>              Loaded                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, UObjectWrapper, HasGetValueTypeHash)

void USpawnUI_C::OnLoaded_3E84B3BE41EA3801C29ED5ABF7BB70A9(TSubclassOf<class UObject> Loaded)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "OnLoaded_3E84B3BE41EA3801C29ED5ABF7BB70A9");

	Params::SpawnUI_C_OnLoaded_3E84B3BE41EA3801C29ED5ABF7BB70A9 Parms{};

	Parms.Loaded = Loaded;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.OnPreviewKeyDown
// (Event, Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// const struct FGeometry&                 MyGeometry                                             (BlueprintVisible, BlueprintReadOnly, Parm, IsPlainOldData, NoDestructor)
// const struct FKeyEvent&                 InKeyEvent                                             (BlueprintVisible, BlueprintReadOnly, Parm)
// struct FEventReply                      ReturnValue                                            (Parm, OutParm, ReturnParm)

struct FEventReply USpawnUI_C::OnPreviewKeyDown(const struct FGeometry& MyGeometry, const struct FKeyEvent& InKeyEvent)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "OnPreviewKeyDown");

	Params::SpawnUI_C_OnPreviewKeyDown Parms{};

	Parms.MyGeometry = std::move(MyGeometry);
	Parms.InKeyEvent = std::move(InKeyEvent);

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function SpawnUI.SpawnUI_C.OnResetToDefault
// (Event, Public, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::OnResetToDefault()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "OnResetToDefault");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.RemoveBed
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   SpawnPointID                                           (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::RemoveBed(int32 SpawnPointID)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "RemoveBed");

	Params::SpawnUI_C_RemoveBed Parms{};

	Parms.SpawnPointID = SpawnPointID;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.RemovedFromViewport
// (Event, Public, BlueprintEvent)

void USpawnUI_C::RemovedFromViewport()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "RemovedFromViewport");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.Settings-UpdateHistory
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    ReviseHistory_                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const struct FS_CC_Settings&            S_CC_Settings                                          (BlueprintVisible, BlueprintReadOnly, Parm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::Settings_UpdateHistory(bool ReviseHistory_, const struct FS_CC_Settings& S_CC_Settings)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Settings-UpdateHistory");

	Params::SpawnUI_C_Settings_UpdateHistory Parms{};

	Parms.ReviseHistory_ = ReviseHistory_;
	Parms.S_CC_Settings = std::move(S_CC_Settings);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.SetupMultiBedButtons
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// const TArray<int32>&                    AverrageIDSSpan                                        (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm)
// int32                                   IdSpawnSelected                                        (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class UBedSpawnWidget_ASA_C*            Bed                                                    (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::SetupMultiBedButtons(const TArray<int32>& AverrageIDSSpan, int32 IdSpawnSelected, class UBedSpawnWidget_ASA_C* Bed)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "SetupMultiBedButtons");

	Params::SpawnUI_C_SetupMultiBedButtons Parms{};

	Parms.AverrageIDSSpan = std::move(AverrageIDSSpan);
	Parms.IdSpawnSelected = IdSpawnSelected;
	Parms.Bed = Bed;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.SetupNameValueOnSlider
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// class USlider*                          Slider                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)
// const class FText&                      InText                                                 (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm)

void USpawnUI_C::SetupNameValueOnSlider(class USlider* Slider, const class FText& InText)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "SetupNameValueOnSlider");

	Params::SpawnUI_C_SetupNameValueOnSlider Parms{};

	Parms.Slider = Slider;
	Parms.InText = std::move(InText);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.SetupUpdateBedSpawn
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// const struct FVector&                   Location                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   Index_0                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   SpawnPointID                                           (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// double                                  NextAllowedUseTime                                     (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// bool                                    bAllowedUse                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   NumBeds                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const class FString&                    BedName                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, HasGetValueTypeHash)

void USpawnUI_C::SetupUpdateBedSpawn(const struct FVector& Location, int32 Index_0, int32 SpawnPointID, double NextAllowedUseTime, bool bAllowedUse, int32 NumBeds, const class FString& BedName)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "SetupUpdateBedSpawn");

	Params::SpawnUI_C_SetupUpdateBedSpawn Parms{};

	Parms.Location = std::move(Location);
	Parms.Index_0 = Index_0;
	Parms.SpawnPointID = SpawnPointID;
	Parms.NextAllowedUseTime = NextAllowedUseTime;
	Parms.bAllowedUse = bAllowedUse;
	Parms.NumBeds = NumBeds;
	Parms.BedName = std::move(BedName);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.SetupUpdateSpawnRegion
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// const struct FVector&                   Location                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   Index_0                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   Difficult                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::SetupUpdateSpawnRegion(const struct FVector& Location, int32 Index_0, int32 Difficult)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "SetupUpdateSpawnRegion");

	Params::SpawnUI_C_SetupUpdateSpawnRegion Parms{};

	Parms.Location = std::move(Location);
	Parms.Index_0 = Index_0;
	Parms.Difficult = Difficult;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.ShowModIcon
// (Public, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::ShowModIcon()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "ShowModIcon");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.Shuffle-SliderValues
// (Public, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::Shuffle_SliderValues()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Shuffle-SliderValues");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.SizeZoomUpdate
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::SizeZoomUpdate()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "SizeZoomUpdate");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.SwitchTheButton
// (Event, Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    Next                                                   (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::SwitchTheButton(bool Next)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "SwitchTheButton");

	Params::SpawnUI_C_SwitchTheButton Parms{};

	Parms.Next = Next;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.Tick
// (BlueprintCosmetic, Event, Public, BlueprintEvent)
// Parameters:
// const struct FGeometry&                 MyGeometry                                             (BlueprintVisible, BlueprintReadOnly, Parm, IsPlainOldData, NoDestructor)
// float                                   InDeltaTime                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::Tick(const struct FGeometry& MyGeometry, float InDeltaTime)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "Tick");

	Params::SpawnUI_C_Tick Parms{};

	Parms.MyGeometry = std::move(MyGeometry);
	Parms.InDeltaTime = InDeltaTime;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.UpdateBedButton
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// double                                  OldTime                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   CustomID                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// bool                                    CanUse                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class UDataListButton_ASA_C*            BedButton                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)
// const class FString&                    BedName                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, HasGetValueTypeHash)

void USpawnUI_C::UpdateBedButton(double OldTime, int32 CustomID, bool CanUse, class UDataListButton_ASA_C* BedButton, const class FString& BedName)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "UpdateBedButton");

	Params::SpawnUI_C_UpdateBedButton Parms{};

	Parms.OldTime = OldTime;
	Parms.CustomID = CustomID;
	Parms.CanUse = CanUse;
	Parms.BedButton = BedButton;
	Parms.BedName = std::move(BedName);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.UpdateBodyPartByType
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// const class FString&                    A                                                      (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, HasGetValueTypeHash)
// const class FString&                    B                                                      (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, HasGetValueTypeHash)
// float                                   Value                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::UpdateBodyPartByType(const class FString& A, const class FString& B, float Value)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "UpdateBodyPartByType");

	Params::SpawnUI_C_UpdateBodyPartByType Parms{};

	Parms.A = std::move(A);
	Parms.B = std::move(B);
	Parms.Value = Value;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.UpdateCameraPerTab
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::UpdateCameraPerTab()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "UpdateCameraPerTab");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.UpdateCharacterFirstSpawnBP
// (Event, Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    Active                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::UpdateCharacterFirstSpawnBP(bool Active)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "UpdateCharacterFirstSpawnBP");

	Params::SpawnUI_C_UpdateCharacterFirstSpawnBP Parms{};

	Parms.Active = Active;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.UpdateHairCameraOffset
// (BlueprintCallable, BlueprintEvent)

void USpawnUI_C::UpdateHairCameraOffset()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "UpdateHairCameraOffset");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.UpdateHairLenghtSettings
// (Public, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::UpdateHairLenghtSettings()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "UpdateHairLenghtSettings");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.UpdateScalarParameter
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class FName                             ParameterName                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// float                                   Value                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::UpdateScalarParameter(class FName ParameterName, float Value)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "UpdateScalarParameter");

	Params::SpawnUI_C_UpdateScalarParameter Parms{};

	Parms.ParameterName = ParameterName;
	Parms.Value = Value;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpawnUI.SpawnUI_C.UpdateSelectedBed
// (Public, BlueprintCallable, BlueprintEvent)

void USpawnUI_C::UpdateSelectedBed()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "UpdateSelectedBed");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpawnUI.SpawnUI_C.UpdateVectorParameter
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class FName                             ParameterName                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// float                                   R                                                      (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// float                                   G                                                      (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// float                                   B                                                      (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// float                                   A                                                      (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpawnUI_C::UpdateVectorParameter(class FName ParameterName, float R, float G, float B, float A)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpawnUI_C", "UpdateVectorParameter");

	Params::SpawnUI_C_UpdateVectorParameter Parms{};

	Parms.ParameterName = ParameterName;
	Parms.R = R;
	Parms.G = G;
	Parms.B = B;
	Parms.A = A;

	UObject::ProcessEvent(Func, &Parms);
}


SDK_NAMESPACE_END
